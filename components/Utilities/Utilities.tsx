...
try {
  return (
    <>
      ...
      <Modal opened={open} onClose={() => setOpen(false)} title="Account Info">
        <Card shadow="sm" p="lg" radius="md" withBorder>
        <Group position="apart" mt="md" mb="xs">
        <TitleText title={`${user?.username}`} />
        <TitleText title={`${user?.role === "MD" ? "Managing Director" : user?.role === "CO" ? "Credit Officer" : user?.role === "CA" ? "Credit Admin" : "Auditor" }`} />
        </Group>
          <Card.Section>
          </Card.Section>
        </Card>
