...
const CreateMember = ({ email }: { email: string; status: string }) => {
  ...
  const handle = trpc.transactions.create.useMutation({
    onSuccess: async () => {
      ...
      try {
        if (
          ...
          handle.mutate({
            ...
            transID: form.values.mpesaCode.toUpperCase(),
            transAmount: form.values.membershipAmount.toUpperCase(),
            transTime: "00000000000000",
            businessShortCode: "4085055",
            billRefNumber: "MEMBERSHIP",
            invoiceNumber: "",
            orgAccountBalance: "0",
            thirdPartyTransID: "",
            msisdn: form.values.phoneNumber,
            firstName: form.values.firstName,
            middleName: form.values.lastName.split(" ")[0] ?? "",
            lastName: form.values.lastName.split(" ")[1] ?? "",
            handlerId: `${user?.id}`,
            updaterId: `${user?.id}`,
            payment: `membership`,
            state: `handled`,
